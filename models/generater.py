...
TASK_ID2STRING={"Onto":18,"conll03":4,"ace2004":7,"ace2005":7,'genia':5,'cadec':1,'share2013':1,'share2014':1}
MAX_LABEL_NUM={"Onto":3,"conll03":3,"ace2004":5,"ace2005":5,'genia':5,'cadec':8,'share2013':8,'share2014':8}
START_ENTITY_NUM={"Onto":3,"conll03":3,"ace2004":8,"ace2005":8,'genia':8,'cadec':10,'share2013':8,'share2014':8}
FLAT_ENTITY={"Onto":1,"conll03":1}
NESTED_ENTITY={"ace2004":1,"ace2005":1,"genia":1}
DISCONTINUE_ENTITY={"cadec":1,"share2013":1,"share2014":1}
MAX_Interval={"Onto":1,"conll03":1,"ace2004":1,"ace2005":1,'genia':1,'cadec':15,'share2013':27,'share2014':27}
class SequenceGeneratorModel(nn.Module):
    ...
    def __init__(self, seq2seq_model: Seq2SeqModel, bos_token_id, eos_token_id=None, max_length=30, max_len_a=0.0,
                 ...
                 restricter=None,dataset_name=None,use_part_label=True,target_type='bpe'):
        """
        ...
        if use_part_label:
            self.target_shift = 2+TASK_ID2STRING[dataset_name]+1
        else:#不使用部分实体标签
            self.target_shift = 2 + TASK_ID2STRING[dataset_name]
        self.real_target_shift = 2+TASK_ID2STRING[dataset_name]
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len=None, tgt_seq_len=None, mask_query=None, OOV_con=None,
                all_word_entity_label=None,target_mask=None,cal_label_mask=None):
        """
        ...
        :return:
        """
        return self.seq2seq_model(src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, mask_query, OOV_con,
                                  all_word_entity_label,target_mask,cal_label_mask)

    def predict(self, src_tokens, src_seq_len=None, mask_query=None, OOV_con=None,
                tgt_tokens=None,target_span=None):
        """
        ...
        :return:
        """
        state = self.seq2seq_model.prepare_state(src_tokens, src_seq_len, mask_query=mask_query,
                                                OOV_con=OOV_con)
        ...
        '''然后再抽取非空的实体'''
        result_old = self.generator.generate(state,tgt_tokens)
        result = []
        for i in range(batch_size):  # 把每个batch收录的结果全部放入
            pairs = []
            aim_pred = result_old[i]
            for ps in aim_pred:
                ps = ps.cpu().tolist()
                if len(ps) <= 3:#没有实体
                    continue
                pairs.append(ps[2:-1] +ps[1:2])
                # for j in ps:
                #     if j < self.target_shift:  # target_shift==3
                #         if len(cur_pair) > 0 and j < self.real_target_shift and j != self.eos_token_id:
                #             if all([cur_pair[i1] < cur_pair[i1 + 1] for i1 in range(len(cur_pair) - 1)]):
                #                 if self.target_type=='bpe':
                #                     while bpe_tail_flag[i][cur_pair[-1]]==1:#没有到正确的终点
                #                         cur_pair.append(cur_pair[-1]+1)
...
class SequenceGenerator:
    ...
    def __init__(self, decoder: Seq2SeqDecoder, max_length=20, max_len_a=0.0, num_beams=1,
                 ...
                 repetition_penalty=1, length_penalty=1.0, pad_token_id=0, restricter=None,dataset_name=None):
        """
        ...
        self.generate_func = partial(greedy_generate, decoder=decoder, max_length=max_length, max_len_a=max_len_a,
                                     ...
                                     restricter=restricter,dataset_name=dataset_name)
    ...
    def set_new_generator(self, max_length=-1, max_len_a=-1, num_beams=-1,
                          repetition_penalty=-1, length_penalty=-1, restricter=-1,dataset_name=None):
        if max_length == -1:
...
def greedy_generate(decoder, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=1,
                    ...
                    repetition_penalty=1, length_penalty=1.0, restricter=None,dataset_name=None,Generate_negative_samples=False):
    """
    贪婪地搜索句子
    ...
    if num_beams == 1:
        token_ids = _no_beam_search_generate(decoder, tokens=tokens, state=state, max_length=max_length, max_len_a=max_len_a,
                                             bos_token_id=bos_token_id, eos_token_id=eos_token_id,
                                             repetition_penalty=repetition_penalty, length_penalty=length_penalty,
                                             pad_token_id=pad_token_id, restricter=restricter,dataset_name=dataset_name)
    else:
...
def _no_beam_search_generate(decoder: Seq2SeqDecoder, state, tokens=None, max_length=20, max_len_a=0.0, bos_token_id=None,
                             ...
                             restricter=None,dataset_name=None):
    ...
    batch_size = tokens.size(0) //TASK_ID2STRING[dataset_name]
    ...
    if max_len_a!=0:
        ...
        real_max_length = 50
        if state.encoder_mask is not None:
            max_lengths = state.encoder_mask.new_ones(state.encoder_mask.size(0)).long()*real_max_length
        else:
            max_lengths = tokens.new_full((tokens.size(0),), fill_value=real_max_length, dtype=torch.long)
    token_ids = tokens
    ...
    result_all=[]#表示每个句子中的实体序列
    for i in range(batch_size):
        result_all.append([])

    #重复位置
    all_pos = all_pos.repeat_interleave(TASK_ID2STRING[dataset_name])
    ...
    while cur_len < real_max_length:
        ...
        '''错误样本采样，每次直接前四个都继续采样'''
        # judge_scores = torch.clamp(torch.min(torch.topk(scores, MAX_LABEL_NUM[dataset_name], 1, largest=True)[0], dim=-1)[0], 0, 100)
        # scores_flag = scores > judge_scores.unsqueeze(1)
        scores_flag = scores >0
        #测试，只让终点过
        # scores_flag = torch.zeros_like(scores).eq(1)
        # scores_flag[:,1]=True
        '''常规采样,若是一个序列两个标签该如何呢'''
        scores_flag = scores_flag.view(-1)
        '''对于嵌套实体和flat实体，每次只能得到标签或者下一个词作为下一个点 《9或者 相减=1
        对于不连续的实体，每次也只能选择后15，或者标签作为下一个点'''
        # next_pos = torch.LongTensor(list(range(vocab_size))).unsqueeze(0).expand(new_batch_size, -1).to(
        #     token_ids.device)
        # '''先验条件'''
        # if cur_len == 1:  # 不能生成标签
        #     pre_flag = torch.logical_or((next_pos == 1).view(-1),
        #                                 (next_pos >= 2 + TASK_ID2STRING[dataset_name]).view(-1))
        # elif dataset_name in DISCONTINUE_ENTITY:
        #     pre_flag = torch.logical_or(
        #         (((next_pos - token_ids[:, -1:]) > 0) & (((next_pos - token_ids[:, -1:]) < 20))).view(-1),
        #         (next_pos < 2 + TASK_ID2STRING[dataset_name]).view(-1))
        # else:  # 只能紧挨着
        #     pre_flag = torch.logical_or(((next_pos - token_ids[:, -1:]) == 1).view(-1),
        #                                 (next_pos < 2 + TASK_ID2STRING[dataset_name]).view(-1))
        ...
        '''同样需要更新seq_scores'''
        seq_scores = seq_scores[next_tokens_pos]+scores[next_tokens_pos]
        all_pos = all_pos[next_tokens_pos]
        '''得到max_lengths'''
        ...
        end_mask = next_tokens <(3+TASK_ID2STRING[dataset_name])#只有为终点符号才会输出
        ...
        # 选择那些已经完成了的
        entity_mask = (next_tokens ==1)#
        end_mask_pos = torch.nonzero(entity_mask).squeeze(-1)#已经结束的就不放进去添乱了
        ...
        if len(continue_pos)>0:#还有没有正常结束的序列
            ...
            seq_scores = seq_scores[continue_pos]
