...
class BartSeq2SeqModel(Seq2SeqModel):
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, mask_query=None,OOV_con=None,
        ...
        if (all_word_entity_length > 10).sum()>0:
            ...
            state = BartState(decoder_encoder[pos2][:, 0:mask_lens],
                              decoder_encoder_mask[pos2][:, 0:mask_lens], mask_query[pos2][:, 0:mask_lens+prefix_len]
                              , org_mask[pos2][:, 0:mask_lens],
                              org_embedding[pos2][:, 0:mask_lens], tgt_seq_len[noo_entity_pos2])
            decoder_output[noo_entity_pos2,:,0:mask_lens+prefix_len] = self.decoder(tgt_tokens2, state) # 问题是有可能有些batch没有机会有起点
        '''处理预测的结果+对应的标签'''
        mask = aim_mask[:, 2:] == 0
        mask = ~mask
